...
/**
 * Add a modern signature block using configured signatories.
 * @param {PDFDocument} doc
 * @param {Object} org - Organization settings (optional, for signatory names/titles)
 */
function signatureBlock(doc, org) {
  ...
  const signatories = [];
  if (org && org.signatory1_title) signatories.push({ title: org.signatory1_title, name: org.signatory1_name || '' });
  if (org && org.signatory2_title) signatories.push({ title: org.signatory2_title, name: org.signatory2_name || '' });
  if (org && org.signatory3_title) signatories.push({ title: org.signatory3_title, name: org.signatory3_name || '' });

  // Fallback if no signatories configured
  if (signatories.length === 0) {
    signatories.push({ title: 'Treasurer / Finance Secretary', name: '' });
    signatories.push({ title: 'President / Chairman', name: '' });
  }

  const sigY = doc.y;
  const sigCount = signatories.length;
  const sigWidth = sigCount === 3 ? CONTENT_WIDTH * 0.28 : CONTENT_WIDTH * 0.38;
  const gap = sigCount === 3 ? (CONTENT_WIDTH - sigWidth * 3) / 2 : CONTENT_WIDTH - sigWidth * 2;

  signatories.forEach(function(sig, i) {
    const x = MARGIN + i * (sigWidth + (i > 0 ? gap / (sigCount - 1) : 0));
    const xPos = sigCount === 3
      ? MARGIN + i * (sigWidth + (CONTENT_WIDTH - sigWidth * 3) / 2)
      : i === 0 ? MARGIN : PAGE_WIDTH - MARGIN - sigWidth;

    doc.moveTo(xPos, sigY).lineTo(xPos + sigWidth, sigY).lineWidth(0.5).strokeColor(COLORS.muted).stroke();

    if (sig.name) {
      doc.font('Helvetica-Bold').fontSize(7.5).fillColor(COLORS.dark);
      doc.text(sig.name, xPos, sigY + 5, { width: sigWidth, align: 'center' });
    }
    doc.font('Helvetica').fontSize(7).fillColor(COLORS.muted);
    doc.text(sig.title, xPos, sigY + (sig.name ? 16 : 5), { width: sigWidth, align: 'center' });
  });
